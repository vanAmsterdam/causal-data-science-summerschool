...
In `scm1` replace `f_los` with a specific *value*, e.g. `7` days (notation: $M_x=<U,V,F_x>$)
...
We say that $Q$ is **identifiable** in a class $M$ of models if, for any pairs of models $M_1$ and $M_2$ from $M$,
...
Based on *observed data* and *assumptions* we cannot *identify* our question $Q$,

. . .

i.e. multiple models with different answers for $Q$ fit the observed data equally well

## Not identified means we cannot compute the estimand
...
is Pearl's definition of confounding
