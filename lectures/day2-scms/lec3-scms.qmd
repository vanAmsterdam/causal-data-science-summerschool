...
5. typically: condition on confounders, don't condition on mediators or colliders
...
- However,
    ...
    - SCMs go a level deeper than DAGs, DAGs naturally 'arise' from SCMs
    ...
    - SCM thinking aligns with physical thinking about the world and is a natural way to think about causality
...
- *background variables* (exogenous, `u`nobserved):
...
## Structural Causal Model 1: an R program {#sec-scm1}
...
All our suspects (models) have the same partial observations but different answers to our question
...
## Not identified in a concrete example
...
## Numeric example: same observations, two SCMs

Binary simplification of the slide's DAG: $Z \to T$, $Z \to Y$, $T \to Y$.

Assume $Z$ is latent. Both models have

$$
P(Z=1)=0.5,\quad P(T=1|Z=0)=0.2,\quad P(T=1|Z=1)=0.8
$$
...
Both imply the same observed data over $(T,Y)$:

$$
P(T=1)=0.5,\quad P(Y=1|T=1)=0.64,\quad P(Y=1|T=0)=0.36
$$

## Same observations, different causal answers

If $Z$ were observed, the backdoor estimand would be

$$
P(Y=1|\text{do}(T=t))=\sum_z P(Y=1|T=t,Z=z)P(Z=z)
$$

For $M_A$:

$$
P(Y=1|\text{do}(T=1))=0.40,\quad
P(Y=1|\text{do}(T=0))=0.60
$$
...
$$
P(Y=1|\text{do}(T=1))=0.70,\quad
P(Y=1|\text{do}(T=0))=0.30
$$
...
Same observed $P(T,Y)$, different $Q(M)$.
...
- many more effects are identified in this setting
  - settings with unobserved confounding and positivity violations may still be identified
- caveats:
...
- this is called a *counterfactual* because it's a probability statement incorporating two incompatible worlds: the world where $T=t$ and the world where $T=t'$
- steps:
...
- DAGs:
  ...
  - *selection diagrams / transportability*: ask whether effects learned in one population apply to another
