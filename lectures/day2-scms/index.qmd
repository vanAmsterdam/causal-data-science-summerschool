...
- Question: are hospital deliveries better than home deliveries?
...
## Hotel manager tycoon

you run a hotel (Backdoor Bay Resort) and want to optimize tips by offering complementary drinks

- treatment: offer complementary drink: (no / yes)
- it costs 1eu to offer a drink
- hotel policy: offer drinks on bad weather days (as a consolation)

<!-- drink on good weather day = happy customer, tip=5 -->
...
# t=0: tea, t=1: soda
# z=0: bad weather, z=1: good weather
dnames <- list(drink=c('no', 'yes'), weather=c('bad', 'good'))
pos_tz <- matrix(c(
  c(0, 3), # y|t=0,z=0,1
  c(2, 5)  # y|t=1,z=0,1
), nrow=2, byrow=T,
...
ps_tz <- matrix(c(
  c(0.0, 0.5), # p(t=0,z=0,1)
  c(0.5, 0.0)  # p(t=1,z=0,1)
), nrow=2, byrow=T)
