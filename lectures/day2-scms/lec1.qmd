...
Probability statements about *random events* $A$ ($A=1\to$ patient dies) and $B$ ($B=1\to$ has cancer):
...
::: {#fig-intervention layout="[20, 20, 60]"}
...
- often left on the table: *how* to do statistical inference (much can still go wrong here)
...
## Collider bias happens in the wild

- collider bias happens when conditioning on a collider opens a new path
- may not be too visible: doing an analysis in a selected subgroup is a form of ('invisible') conditioning)
- collider bias may happen e.g. when selecting only patients in the hospital
...
- collider bias *might* be an explanation for the *obsesity paradox* (i.e. obesity is correlated with better outcomes in diverse medical settings) [e.g. @banackObesityParadoxMay2017]

## Basic DAG patterns

::: {#fig-dag-types layout-ncol=3}

![chain / mediation](_tikzs/dag-chain.png)

![fork / confounder](_tikzs/dag-fork.png)

![collider](_tikzs/dag-collider)
