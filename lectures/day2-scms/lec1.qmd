...
## DAGs convey assumptions on conditional independence
...
- @fig-dag-fire2 says *there may be other ways through which `fire` causes `wet floor`*
  ...
  - e.g. @fig-dag-fire3 is also compatible with @fig-dag-fire2
...
- $Z$ = age, $X$ =  shoe size, $Y$ = reading ability
- $X \mathrel{\not\!\perp} Y$ (i.e. $X$ and $Y$ are dependent)
- $X \perp Y | Z$ (for kids with same age, shoe size has no information on reading ability)
...
- $X$: student signs up for causal inference course, $M$: student participates in causal inference course, $Y$: student understands causal inference
- $X \mathrel{\not\!\perp} Y$ (i.e. $X$ and $Y$ are dependent)
- $X \perp Y | M$
...
\begin{align}
    \text{methodological quality} &\sim U[0,1] \\
    \text{interesting results}  &\sim U[0,1] \\
    \text{published}   &= I_{\text{methodological quality} + \text{interesting results} > 1}
\end{align}
...
methodological.quality = runif(n)
interesting.results = runif(n)
s = methodological.quality + interesting.results > 1
df <- data.frame(methodological.quality, interesting.results, s)
df$s <- factor(df$s)

ggplot(df, aes(x=methodological.quality, y=interesting.results)) +
  ...
  scale_color_manual(
    ...
    labels=c('published', 'not published'),
    guide=guide_legend(
      title=''
      )
    ) + 
...
"What is the expected value of $Y$ if we set $T$ to a particular value, say $T=1$?"

- What would the DAG look like if we could intervene on $T$ and set it to 1?
  - this assumes such a *modular* intervention is possible: i.e. leave everything else unaltered
- in the graph, [intervening]{.fg} on variable $T$ means removing all arrows that go into $T$
...
- In the *intervened* DAG, $T$ and $Z$ are **marginally independent**, meaning that $P_{\text{int}}(T|Z) = P_{\text{int}}(T)$
...
### Compare $P(Y|T)$ in both the observational and intervened DAGs.
...
Observational: $P_{\text{obs}}(Z=z|T) \neq P(Z=z)$ ($Z$ and $T$ are dependent).
Sum rule gives us:
...
Interventional: $P_{\text{int}}(Z=z|T) = P(Z=z)$ ($Z$ and $T$ are independent)
...
- $P(\text{outcome}|\text{location}) = \sum_{\text{risk}} P(\text{outcome}|\text{location},\text{risk})P(\text{risk}|\text{location})$
...
## Back to example 1: Doing

::: {layout-ncol=2}

![DAG](_tikzs/dag-delivery-int.png)
...
::::{style="font-size: 0.65em; line-height: 1.15;"}
- $P(\text{outcome}|\text{do}(\text{location})) = \sum_{\text{risk}} P(\text{outcome}|\text{location},\text{risk})P(\text{risk})$
...
## The back-door theorem and adjustment formula
